...
func testDoltIndex(t *testing.T, keys []interface{}, expectedRows []sql.Row, index sql.Index, cmp indexComp) {
	...
	for i, key := range keys {
		switch cmp {
		case indexComp_Eq:
			index = index.Equals(ctx, exprs[i], key)
		case indexComp_NEq:
			index = index.NotEquals(ctx, exprs[i], key)
		case indexComp_Gt:
			index = index.GreaterThan(ctx, exprs[i], key)
		case indexComp_GtE:
			index = index.GreaterOrEqual(ctx, exprs[i], key)
		case indexComp_Lt:
			index = index.LessThan(ctx, exprs[i], key)
		case indexComp_LtE:
			index = index.LessOrEqual(ctx, exprs[i], key)
		default:
			panic("should not be hit")
		}
	}
	indexLookup, err := index.Build(ctx)
	require.NoError(t, err)
	dil, ok := indexLookup.(*doltIndexLookup)
	require.True(t, ok)
	indexIter, err := dil.RowIter(ctx, dil.IndexRowData(), nil)
